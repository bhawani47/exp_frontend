...
export const DataContextProvider = ({ children }) => {
  ...
  const createData = useCallback(
    async (type, transactionData, retryCount = 0) => {
      ...
      try {
        ...
        setPendingUpdates((prev) => [
          ...prev,
          { id: tempId, type: "create", data: newData },
        ]);
        ...
        setError(
          "Failed to create data: " + (error.message || "Unknown error")
        );
  ...
  const deleteItem = useCallback(
    async (id, retryCount = 0) => {
      ...
      try {
        ...
        setPendingUpdates((prev) => [
          ...prev,
          { id: tempId, type: "delete", itemId: id },
        ]);
  ...
  const editData = useCallback(
    async (id, updatedData, retryCount = 0) => {
      if (!id || !token) return false;
      ...
      try {
        ...
        setPendingUpdates((prev) => [
          ...prev,
          { id: tempId, type: "edit", itemId: id, data: formattedData },
        ]);
        ...
        setError(
          "Failed to update data: " + (error.message || "Unknown error")
        );
